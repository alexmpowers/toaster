...
impl HistoryManager {
    ...
    fn save_entry_with_conn(
        ...
        Ok(HistoryEntry {
            ...
            post_processed_text: post_processed_text.map(|s| s.to_string()),
            post_process_prompt: post_process_prompt.map(|s| s.to_string()),
            post_process_requested,
        })
    }
    ...
    #[cfg(test)]
    fn cleanup_by_count_with_conn(conn: &Connection, limit: usize) -> Result<usize> {
        let mut stmt = conn.prepare(
            "SELECT id, file_name FROM transcription_history WHERE saved = 0 ORDER BY timestamp DESC",
        )?;
        ...
        let entries_to_delete = &entries[limit..];
        let mut deleted_count = 0;
        for (id, _file_name) in entries_to_delete {
            conn.execute(
                "DELETE FROM transcription_history WHERE id = ?1",
                params![id],
            )?;
