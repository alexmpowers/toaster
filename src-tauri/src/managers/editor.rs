...
/// Precision eval module.
///
/// These tests are the machine-enforced side of the PRD acceptance gate
/// described in `.github/skills/transcript-precision-eval/SKILL.md`.
/// They are discoverable by name via `cargo test precision_eval`, which is
/// the entry point the `eval-harness-runner` agent invokes (see
/// `.github/agents/eval-harness-runner.md`).
///
/// Scope today:
///   * Per-word timing is never synthesized to equal durations.
///   * Keep-segment arithmetic round-trips through delete/undo byte-for-byte.
///   * Midstream deletion produces a clean splice (no remnant from the
///     deleted region leaks into the post-cut segment).
///
/// Scope deferred (requires human-verified golden JSON fixture):
///   * Whole-pipeline per-word comparison against
///     `tests/fixtures/toaster_example.words.golden.json`.
#[cfg(test)]
mod precision_eval {
    use super::*;
