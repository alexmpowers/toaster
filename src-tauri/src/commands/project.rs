...
pub fn load_project(
    ...
    // from older code paths, so we use the repair path (sanitize) rather
    // than the strict path (new).
    let audio_duration_us = project
        .words
        .last()
        .map(|w| w.end_us + 1_000_000) // 1s buffer past last word
        .unwrap_or(0);
    let validated =
